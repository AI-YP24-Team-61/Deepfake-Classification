...
batch_size = st.select_slider(
    "Выберите размер батча (batch_size)", options=[2**j for j in range(5, 11)]
)
...
weight_decay = st.slider(
    r"Выберите величину коэффициента регуляризации ($\lambda$)",
    min_value=0.0,
    max_value=1.0,
    step=0.01,
)
...
if left.button("Обучить нейронную сеть", use_container_width=True):
    if id != "" and id is not None:
        ...
        if (
            response_fit["message"]
            == f"Model with id '{id}' already exist. Enter another id."
        ):
...
st.header("Инференс обученной модели", divider="gray")
...
if asyncio.run(get_data("models"))[0]:
    ...
    if st.button(
        f"Установить '{set_model_id}' в качестве модели для инференса.",
    ...
    if uploaded_file is not None:
        ...
        if st.button("PREDICT", use_container_width=True, type="primary"):
            predict_response = asyncio.run(
                post_data("predict", input_data={"id": set_model_id})
            )
            st.write(
                f"Вероятность принадлежности изображения к классу REAL: **{predict_response['real_prob']:.2f}**"
            )
