...
st.text('Структура каталога и имена директорий внутри архива должны соответствовать формату ниже:')
st.text("""
        .zip
        ├──dataset
        ├──┴─test
...
lr = st.slider("Выберите размер шага обучения (learning rate)",
               ...
               max_value=0.1,
               step=0.01)
weight_decay = st.slider(r"Выберите величину коэффициента регуляризации ($\lambda$)",
                         min_value=0.,
                         max_value=1.,
                         step=0.01)
...
if asyncio.run(get_data('models'))[0]:
    ...
    if st.button(f"Установить '{set_model_id}' в качестве модели для инференса.",
